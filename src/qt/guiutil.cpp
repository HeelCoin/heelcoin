...
{
    // HeelCoin: check prefix
    if(uri.scheme() != QString("HeelCoin"))
        return false;
...
{
    // Convert HeelCoin:// to HeelCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("HeelCoin://"))
    {
        uri.replace(0, 12, "HeelCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "HeelCoin.lnk";
}
...
{
    return GetAutostartDir() / "HeelCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("HeelCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  HeelCoin-qt [" + tr("command-line options") + "]                     " + "\n";
